...
def func(self, *args, **kwargs):
    ...
    for selected in selected_spot:
        ...
        gContractor_all.append(selected.gContractor)  # 元請会社all
        X_all.append(selected.XSubcontractor)  # 基本装苑all
        Xx_all.append(selected.XxSubcontractor)  # 基本職方all
        # print(spotName)#################
    ...
    for g in gContractor_all:
        for c in g.all():  #################
            # print(c.gName)
            g.gName = c.gName
            g.postalCode = c.postalCode
            g.address = c.address
            g.telephoneNumber = c.telephoneNumber
